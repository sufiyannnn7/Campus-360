...
import { Calendar, Search, MapPin, Users, Filter, Ticket } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Discover and register for what's happening on campus.</p>
        </div>
        <Button className="shrink-0"><Ticket className="mr-2 h-4 w-4" /> My Tickets</Button>
      </div>
              ...
              <Card className="h-full flex flex-col hover-elevate transition-all overflow-hidden border-border/50 shadow-sm cursor-pointer group">
                <div className="h-48 bg-muted relative">
                  ...
                  <div className="absolute top-3 left-3 flex gap-2">
                    <span className={cn("px-2.5 py-1 rounded-md text-xs font-semibold shadow-sm backdrop-blur-md bg-background/95", getEventCategoryColor(event.category))}>
                      {event.category}
                    </span>
                  </div>
                  <div className="absolute top-3 right-3 flex gap-2">
                    <span className={cn("px-2.5 py-1 rounded-md text-xs font-semibold shadow-sm backdrop-blur-md bg-background/95", getStatusColor(event.status))}>
                      {event.status.charAt(0).toUpperCase() + event.status.slice(1)}
                    </span>
                  </div>
                </div>
                <CardContent className="p-5 flex-1 flex flex-col">
                  <h3 className="font-bold text-xl line-clamp-2 group-hover:text-primary transition-colors leading-tight mb-3">
                    {event.title}
                  </h3>
                  
                  <div className="space-y-2 mt-auto">
                    <div className="flex items-center text-sm text-muted-foreground">
                      <Calendar className="mr-2 h-4 w-4 shrink-0 text-primary/70" />
                      <span className="truncate">{formatDateTime(event.startDatetime)}</span>
                    </div>
                    {event.venue && (
                      <div className="flex items-center text-sm text-muted-foreground">
                        <MapPin className="mr-2 h-4 w-4 shrink-0 text-primary/70" />
                        <span className="truncate">{event.venue.name}</span>
                      </div>
                    )}
                    <div className="flex items-center text-sm text-muted-foreground">
                      <Users className="mr-2 h-4 w-4 shrink-0 text-primary/70" />
                      <span className="truncate">{event.registrationCount || 0} registered</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 pt-4 border-t flex items-center justify-between">
                     <div className="text-sm font-medium flex items-center gap-2">
                       {event.club && (
                         <>
                           {event.club.logoUrl ? (
                             <img src={event.club.logoUrl} className="w-5 h-5 rounded-full" alt="" />
                           ) : (
                             <div className="w-5 h-5 rounded-full bg-primary/20 text-primary flex items-center justify-center text-[10px]">{event.club.name.charAt(0)}</div>
                           )}
                           <span className="truncate max-w-[120px]">{event.club.name}</span>
                         </>
                       )}
                     </div>
